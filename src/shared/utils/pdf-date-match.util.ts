...
export function getCanonicalDateToken(date: DateParts): string {
  const dayPadded = String(date.day).padStart(2, "0");
  const monthPadded = String(date.month).padStart(2, "0");
  const shortYearPadded = String(date.year % 100).padStart(2, "0");

  return `${dayPadded}${monthPadded}${shortYearPadded}`;
}

export function extractDateDigitTokens(text: string): string[] {
  return text.match(/\d{4,8}/g) ?? [];
}
...
export function matchesDateFuzzy(date: DateParts, candidateToken: string): boolean {
  ...
  if (!/^\d{4,8}$/.test(normalizedCandidate)) {
    return false;
  }

  if (buildDdMmYyVariants(date).includes(normalizedCandidate)) {
    return true;
  }
  ...
  return isFuzzyDateTokenMatch(getCanonicalDateToken(date), normalizedCandidate);
}
...
export function isFuzzyDateTokenMatch(expectedToken: string, candidateToken: string): boolean {
  ...
  if (
    ...
    && levenshteinDistance(expectedToken, candidateToken) <= MAX_DATE_EDIT_DISTANCE
  ) {
    return true;
  }
